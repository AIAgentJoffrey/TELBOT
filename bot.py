...
async def go_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /go command sent as a true Telegram command.

    This function mirrors the logic of handling "/go" within the text handler,
    but is needed because the text handler excludes commands via its filter.
    ...
    # Retrieve the session for this chat
    ...
    # Build the script using the helper
    script = make_script(
        session["product"],
        session["audience"],
        session["tone"],
    ...
    # Execute the video generation flow
    try:
...
def main() -> None:
    ...
    # Handle the /go command explicitly. Without this, /go messages are
    # considered commands and are not routed to the text handler.
    app.add_handler(CommandHandler("go", go_cmd))
    # All other text (non-command) messages are processed by the main text handler.
    app.add_handler(
